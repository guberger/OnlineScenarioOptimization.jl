...
function generate_risk_samples(N_set, n_sample)
    ...
    for k = 1:n_sample
        N = rand(N_set)
        x_opt = solve_problem(2.5, sample_constraints(N))
...
plot!(plt, xlabel=L"v", ylabel=L"f_\theta(v,N)",
      size=(600, 450), labelfontsize=18, tickfontsize=12, dpi=600)
...
for t = 1:n_step
    x_opt = solve_problem(2.5, sample_constraints(size_list[t]))
    ϵ = compute_risk(2.5, x_opt)
    push!(risk_list, ϵ)
...
for t = 1:n_step
    xobs1 = 2.5 + sin(0.1 * t)
    x_opt = solve_problem(xobs1, sample_constraints(size_list[t]))
